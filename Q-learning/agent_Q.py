...
class AgentPrediction_Q:
	...
	def updateQFn(self,oldObs,oldAct,oldReward,newObs ):
		## implementing the Q update 
		self.QFunction[oldObs][oldAct] = self.QFunction[oldObs][oldAct] + self.alpha* (oldReward+self.gamma*np.max(self.QFunction[newObs]) -self.QFunction[oldObs][oldAct]) 
