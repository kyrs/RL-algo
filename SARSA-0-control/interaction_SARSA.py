...
def mainInt(env,agent):
	...
	for epCnt in range(1,noEpisode):
		...
		for i in range(maxStepPrEps):
			...
			nextAction = agent.action(newObservation,1/epCnt)
			...
			agent.updateQFn(oldObs=observation,oldAct=action,oldReward=reward,newObs=newObservation,newAct=nextAction)
